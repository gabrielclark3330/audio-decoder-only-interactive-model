...
def train(fabric, state, train_dataloader, val_dataloader, monitor, resume):
    ...
    for  train_data in train_dataloader:
        ...
        input_ids, targets = train_data #.contiguous()
...
def validate(fabric: L.Fabric, model: torch.nn.Module, val_dataloader: DataLoader) -> torch.Tensor:
    ...
    for k, val_data in enumerate(val_dataloader):
        ...
        input_ids, targets = val_data#.contiguous()
        logits = model(input_ids)
        loss = chunked_cross_entropy(logits, targets, chunk_size=0)
...
def create_dataloaders(
    ...
    train_dataset = NumpyAudioDataset(val_file_paths, block_size=32, process_rank=0, num_processes=1)
    train_dataloader = DataLoader(dataset, batch_size=2, shuffle=True, prefetch_factor=None)
    val_dataset = NumpyAudioDataset(val_file_paths, block_size=32, process_rank=0, num_processes=1)
    val_dataloader = DataLoader(dataset, batch_size=2, shuffle=True, prefetch_factor=None)
    return train_dataloader, val_dataloader
